...
ZelzalWF_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼 - اوامـر الـردود / والتـرحيب](t.me/Source_Theo) 𓆪\n\n"
...
async def on_snip_list(event):
    ...
    for filt in filters:
        if OUT_STR == "** ❈╎لاتوجـد ردود محفوظـه في هـذه الدردشـه ༗**":
            OUT_STR = "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼 - قائمـة الـردود 𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def _(event):
    ...
    if gvarstatus("TIME_STOP") is not None: #Code by T.me/XIX_A
