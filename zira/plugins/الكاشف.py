...
ZelzalPH_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼 📲 - كـاشـف الارقـام العربيــة](t.me/Source_Theo) 𓆪\n\n"
    "**⪼ الامــر ↵**\n\n"
...
async def _(event): #Code by T.me/XIX_A
    ...
    zelzal = "@ZZIIIbot" #Code by T.me/XIX_A
    zed = await edit_or_reply(event, "**⎉╎جـارِ الكشـف عن الرقـم 📲**\n**⎉╎انتظـر قليـلاً ... ▬▭**")
    async with borg.conversation(zelzal) as conv: # code by t.me/XIX_A
        try:
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(number)
            Source_Theo = await conv.get_response()
            malath = Source_Theo.text
            await borg.send_message(event.chat_id, Source_Theo)
            await zed.delete()
