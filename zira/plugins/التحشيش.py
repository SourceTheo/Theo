...
zed_dev = (6367618993, 6367618993, 6367618993)
zel_dev = (6367618993, 6367618993, 6367618993, 6367618993)
...
async def ge(user, event):
    ...
    try:
        ...
        return None
    return user_obj
########################  Source_Theo     Userbot ~ By: Zelzal (@XIX_A    )  ########################
@zedub.zed_cmd(pattern="رابط الحذف")
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝙝𝙚𝙤    - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/Source_Theo    ) 🗑♻️𓆪\n**𓍹━─━─━─━─𝙏𝙝𝙚𝙤   ─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
########################  Source_Theo     Userbot ~ By: Zelzal (@XIX_A    )  ########################
...
########################  Source_Theo     Userbot ~ By: Zelzal (@XIX_A    )  ########################
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/Source_Theo    ) 𓆪\n\n"
...
async def permalink(malath):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ لك هـذا مرسليو  . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
