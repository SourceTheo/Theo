...
# Theo UsetBot T.me/Source_Theo 
# Devolper Marcelo T.me/XIX_A
import asyncio
import contextlib
import io
import math
...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"ᯓ 𝗧𝗵𝗲𝗼 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - معلومـات الملصـق 🎟\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
...
async def cb_sticker(event):
    split = event.pattern_match.group(1) #Code by T.me/XIX_A
    if "sex" in split or "pussy" in split or "rape" in split or "fuk" in split or "عير" in split or "كس " in split or "penis" in split or "كحب" in split or "قحب" in split or "كحاب" in split:
        return await event.client.send_message("@XIX_A", f"- Cmd : مصلقات {split} 🔞")
    if not split:
...
async def zelzal_gif(event):
    ...
    chat = "@Stickers" #Code by T.me/XIX_A
    ...
    async with borg.conversation(chat) as conv: #Code by T.me/XIX_A
        try:
            ...
            await conv.get_response()
            await conv.send_file(reply) #Code by T.me/XIX_A
            await conv.get_response()
